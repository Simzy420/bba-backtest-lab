...
Implements the FULL production strategy with:
  ...
  - Round 4 Test D ONE KNOB: flatten adverse funding after ≥48h unless ≥ +1R
...
# Round 4 Test D — ONE KNOB ONLY (do not change BE stops / entries / other exits)
# 1R definition: engine has no protective-stop R unit. Official probe→confirm
# promotion already treats +3% from avg entry as the working-trade threshold, so
# 1R = 3% of current avg entry (same unit as hl_backtest.py confirm gate).
FUNDING_FLATTEN_MIN_HOLD_DAYS = 2   # ≥48h; daily bars → ≥2 calendar days held
ONE_R_PCT = 3.0                     # 1R = 3% of avg entry
...
def run_backtest(all_data: dict, regime_df: pd.DataFrame):
    ...
    for i, date in enumerate(dates):
        ...
        for pos in list(positions):
            ...
            # 5. Round 4D: flatten adverse funding after ≥48h unless ≥ +1R
            # Existing exits (MA50 / RSI / time / BE / regime / DD) are unchanged
            # and take priority on the same bar. 1R = ONE_R_PCT of avg entry.
            else:
                avg_entry = pos.avg_entry_price()
                funding_so_far = calc_funding_cost(pos.symbol, pos.total_notional(), hold_days)
                unrealized_pct = ((r["close"] - avg_entry) / avg_entry * 100) if avg_entry else 0.0
                if (hold_days >= FUNDING_FLATTEN_MIN_HOLD_DAYS
                        and funding_so_far > 0
                        and unrealized_pct < ONE_R_PCT):
                    exit_price = r["close"]
                    reason = "FUNDING_FLATTEN"
                    print(f"  [{date.date()}] FUNDING_FLATTEN {pos.symbol} "
                          f"hold={hold_days}d funding=${funding_so_far:.2f} "
                          f"unreal={unrealized_pct:.2f}% < {ONE_R_PCT:.1f}% 1R")
...
def main():
    ...
    print("ONE-KNOB: flatten adverse funding after ≥2 calendar days if < +1R (3% avg entry)")
    print("=" * 80)
    print()
    ...
    results = {
        ...
        "config": {
            ...
            "funding_flatten_min_hold_days": FUNDING_FLATTEN_MIN_HOLD_DAYS,
            "one_r_pct": ONE_R_PCT,
        },
