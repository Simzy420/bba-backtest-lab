#!/usr/bin/env python3
"""
Round 4 Test D — isolated COST CONTROL knob on official R3 Hyperliquid engine.
================================================================================
ONE KNOB ONLY vs bba-engine/hl_backtest.py:
  Flatten longs with adverse (net-cost) funding after hold_time >= 48h
  unless unrealized P&L is already >= +1R.

1R definition (engine has no explicit initial risk stop):
  Probe confirmation / premature BE is armed at ~3% probe gain.
  Therefore 1R = 3% of avg_entry applied to current notional
  (dollar 1R = 0.03 * total_notional). Unrealized R uses GROSS price P&L
  vs that 1R (funding is the cost being cut, not part of the R tape).

NOT changed: BE stops, entries, size, regime filters, universe, costs schedule,
pyramiding, MA/RSI exits, time stop, DD defense.

Run: python3 /workspace/grok-engine/round4d_funding_flatten.py
"""
...
# Round 4D one-knob: flatten adverse funding after 48h unless >= +1R
FUNDING_FLATTEN_HOLD_DAYS = 2  # daily bars: hold_days >= 2 == >= 48 calendar hours
ONE_R_PCT = 0.03               # 3% of avg_entry; proxy matching confirm/BE arm at ~3% probe gain
...
OUTPUT_JSON = "/workspace/grok-results/round4_D_funding_flatten_raw.json"
...
def one_r_dollars(pos):
    """Dollar 1R = 3% of avg_entry * (notional / avg_entry) = 3% of current notional.

    Official R3 engine never stores an initial hard stop. The only explicit
    risk distance in the pyramid is the +3% probe-gain gate that arms confirm/BE.
    Use that 3% as the 1R proxy (documented in knob_detail).
    """
    return pos.total_notional() * ONE_R_PCT


def r_multiple_unrealized(pos, price):
    """Gross price R-multiple: ((price - avg_entry) / avg_entry) / ONE_R_PCT."""
    avg = pos.avg_entry_price()
    if avg <= 0 or ONE_R_PCT <= 0:
        return 0.0
    return ((price - avg) / avg) / ONE_R_PCT
...
def run_backtest(all_data: dict, regime_df: pd.DataFrame):
    ...
    for i, date in enumerate(dates):
        ...
        for pos in list(positions):
            ...
            # 5. Round 4D ONLY: flatten adverse funding after 48h unless >= +1R
            # Existing exits above keep priority on the same bar (rules unchanged).
            elif hold_days >= FUNDING_FLATTEN_HOLD_DAYS:
                funding_so_far = calc_funding_cost(pos.symbol, pos.total_notional(), hold_days)
                # Adverse = cumulative funding is a net cost (engine longs always pay avg rate).
                if funding_so_far > 0 and r_multiple_unrealized(pos, r["close"]) < 1.0:
                    exit_price = r["close"]
                    reason = "FUNDING_FLATTEN"
                    print(
                        f"  [{date.date()}] FUNDING_FLATTEN {pos.symbol} "
                        f"hold={hold_days}d R={r_multiple_unrealized(pos, r['close']):.2f} "
                        f"funding=${funding_so_far:.2f}"
                    )
...
def main():
    print("=" * 80)
    print("ROUND 4 TEST D — FUNDING_FLATTEN after 48h unless >= +1R")
    print("BIG BRAIN APE — DRUCKENMILLER STRATEGY BACKTEST ON HYPERLIQUID PERPS")
    print("ONE-KNOB: flatten adverse-funding longs after 48h if unrealized < +1R")
    print("1R proxy: 3% of avg_entry (confirm/BE arm threshold); BE stops UNCHANGED")
    print("=" * 80)
    print()
    ...
    results = {
        ...
        "config": {
            ...
            "round4d_knob": {
                "funding_flatten_hold_days": FUNDING_FLATTEN_HOLD_DAYS,
                "one_r_pct": ONE_R_PCT,
                "one_r_definition": "3pct_of_avg_entry_notional_proxy_no_explicit_initial_stop",
            },
        },
