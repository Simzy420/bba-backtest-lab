...
Round 4 R1 ONE-KNOB: ban confirm + jugular adds while regime is TRANSITION.
  Probes allowed in TRANSITION; confirm/jugular only when RISK-ON.
  Do not change BE stops, funding, size, universe, fees, or other rules.

Run: python3 grok-engine/round4_r1_transition_probe_only.py
"""
...
OUTPUT_JSON = "/workspace/repos/bba-backtest-lab/grok-results/round4_R1_transition_probe_only_raw.json"
...
def run_backtest(all_data: dict, regime_df: pd.DataFrame):
    ...
    for i, date in enumerate(dates):
        ...
        # Pyramiding: confirm/jugular ONLY in RISK-ON (Round 4 R1: TRANSITION probe-only)
        # Probes may still enter in TRANSITION; confirm/jugular adds banned while TRANSITION.
        if regime == "RISK-ON" and not defensive_mode:
            for pos in positions:
...
def main():
    print("=" * 80)
    print("ROUND 4 R1 — TRANSITION probe-only (ban confirm/jugular in TRANSITION)")
    print("BIG BRAIN APE — DRUCKENMILLER STRATEGY BACKTEST ON HYPERLIQUID PERPS")
    print("ONE-KNOB: confirm+jugular adds only when RISK-ON; probes still allowed in TRANSITION")
    print("=" * 80)
    print()
